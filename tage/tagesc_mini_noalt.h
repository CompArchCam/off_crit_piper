...
int predictorsize() {
  ...
  STORAGESIZE += MINITAGE__NBANK * (1 << MINITAGE__LOGG) *
                 ...
                 MINITAGE__ASSOC;
  STORAGESIZE += MINITAGE__ALTWIDTH;
  // the use_alt counter
  STORAGESIZE +=
      (1 << MINITAGE__LOGB) +
...
#define MINITAGE__NNHIST 14
  ...
  // void Tagepred (uint64_t  PC)
  {
    ...
    BANK1 = (hash_64(PC) ^ (hist_to_use.phist & ((1 << m[1]) - 1))) %
            MINITAGE__NBANK;
    ...
    BI = hash_64(PC) & ((1 << MINITAGE__LOGB) - 1);
    for (int i = 1; i <= MINITAGE__NHIST; i++) {
    ...
    if (HitBank > 0) {
      ...
      if (PseudoNewAlloc) {
        ...
        if (LongestMatchPred != HCpred) {
          ...
          ctrupdate(use_alt_on_na, (HCpred == resolveDir), MINITAGE__ALTWIDTH);
          // pure TAGE only
        }
      }
    }
