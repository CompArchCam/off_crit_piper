...
#ifndef _TAGE_PREDICTOR_192_H_
#define _TAGE_PREDICTOR_192_H_
#include <array>
#include <assert.h>
#include <inttypes.h>
#include <iostream>
...
#define UWIDTH 2
#define LOGASSOC                                                               \
  1 // but partial skewed associativity (option PSK) might be interesting
...
// counters but HYSTSHIFT=0 brings similar accuracy
...
#define REPSK 0
#endif
#define PROTECTRECENTALLOCUSEFUL                                               \
...
#define FORCEONHIGHCONF //   if TAGE is high conf and SC very low conf then use
                        //   TAGE
// Add the extra SC tables that uses some information that do not flow directly
// from TAGE
#define SCMEDIUM
#ifdef SCMEDIUM
#define SCGH
...
//  LongestMatchPred and HCpred are used. 4 SCSUM are computed, and a final
//  4-to-1 selects the correct prediction:   each extra bit of information
//  (confidence, etc) would necessitate  doubling the number of computed SCSUMs
//  and double the width of the final MUX
...
#define PCBL PCBR
#define INDBIASPC                                                              \
  (((((PCBL ^ (PCBL >> (LOGBIAS - 5))))) & ((1 << (LOGBIAS)) - 1)))
#define INDBIASPCLMAP                                                          \
  (((INDBIASPC) ^ ((LongestMatchPred ^ (HCpred << 1)) << (LOGBIAS - 2)))) &    \
      ((1 << (LOGBIAS)) - 1)
// a single  physical table but  two logic tables: indices agree on all the bits
// except 2

#define INDBIASIMLIBR                                                          \
  (((((PCBL ^ hist_to_use.F_BrIMLI ^ (PCBL >> (LOGBIAS - 6))))) &              \
    ((1 << LOGINB) - 1)))
#define INDBIASIMLITA                                                          \
  ((((((PCBL >> 4) ^ hist_to_use.F_TaIMLI ^ (PCBL << (LOGBIAS - 4))))) &       \
    ((1 << LOGINB) - 1)))

#define INDSBIASIMLIBR                                                         \
  (((((PCBL ^ (PCBL >> 6) ^ hist_to_use.F_BrSIMLI ^                            \
       (PCBL >> (LOGBIAS - 6))))) &                                            \
    ((1 << LOGINB) - 1)))
#define INDSBIASIMLITA                                                         \
  ((((((PCBL >> 5) ^ hist_to_use.F_TaSIMLI ^ (PCBL << (LOGBIAS - 5))))) &      \
    ((1 << LOGINB) - 1)))
...
#define INDBIASALTTAGE                                                         \
  ((LongestMatchPred == alttaken) + (TAGECONF << 1) +                          \
   ((ALTCONF >= TAGECONF) << 3) + (LongestMatchPred << 4) +                    \
   (((DIFFBANK > 4) + (DIFFBANK > 9) + (DIFFBANK > 13)) << 5))
int8_t BiasBIM[(1 << 6)];
#define INDBIASBIM                                                             \
  ((LongestMatchPred == BIMPRED) + ((BIMCONF >= TAGECONF) << 1) +              \
   (((HitBank > 0) + (HitBank > 7) + (HitBank > 13)) << 2) +                   \
   ((AltBank > 0) << 4)) +                                                     \
      (LongestMatchPred << 5)
int8_t BiasLMHCALT[(1 << 9)]; // in practice less entries !!
#define INDBIASLMHCALT                                                         \
  (LongestMatchPred + (HCpred << 1) + (alttaken << 2) + (TAGECONF << 3) +      \
   (ALTCONF << 5) + (HCCONF << 7))
#endif
...
// second local history:in fact history within the same instruction block of
// instruction
...
// fourth local history  incorporates 2 bits of global history and the current
// direction in the "local history"
...
#define INDUPDS0                                                               \
  ((PCBR ^ (PCBR >> 5) ^ (PCBR >> 3)) & ((1 << (LOGSIZEUPS)) - 1))

// provide extra multiplicative weight to emphasise/deemphasize the contribution
// of IMLI and local components
...
int8_t COUNT50[(1 << LOGCOUNT)][(NHIST / 4) + 1] = {
    {7}}; // more or less than 50%  misprediction on weak LongestMatchPred
int8_t COUNT16_31[(1 << LOGCOUNT)][(NHIST / 4) + 1] = {
...
uint GGI[ASSOC]
        [NHIST + 1];  // indexes to the different ways  for tables  in TAGE
...
struct cbp_hist_t {
  ...
  uint64_t PCBLOCK;
  //////////////////////IMLI RELATED and backward/Forward
  /// history////////////////////////////////////
  uint64_t TaIMLI;   // use to monitor the iteration number (based on target
                     // locality for backward branches) Region 64 bytes
  uint64_t BrIMLI;   // use to monitor the iteration number (a second version
                     // based on backward branch locality)) Region 64 bytes
  uint64_t F_TaIMLI; // use to monitor the iteration number,BHIST if TaIMLI = 0
  uint64_t F_BrIMLI; // use to monitor the iteration number (a second version),
                     // FHIST if BrIMLI = 0

  uint64_t TaSIMLI; // use to monitor the iteration number (based on target
                    // locality for backward branches) Region 4 bytes
  uint64_t BrSIMLI; // use to monitor the iteration number (a second version
                    // based on backward branch locality)) Region 4 bytes
  uint64_t
      F_TaSIMLI; // use to monitor the iteration number,BHIST if TaSIMLI = 0
  uint64_t F_BrSIMLI; // use to monitor the iteration number (a second version),
                      // FHIST if BrSIMLI = 0
...
#ifdef EXTRAW
  inter +=
      2 * PERCWIDTH * (1 << LOGSIZEUPS); // the 2 EXTRAW   table multipliers
#endif

  STORAGESIZE += inter;
  fprintf(stderr, "(IMLI %d)", inter);
  inter = 0;
#endif
...
#define NNHIST 36
    ...
    for (int i = 2; i <= NNHIST; i++) {
      mm[i] = (int)(((double)MINHIST *
                     pow((double)(MAXHIST) / (double)MINHIST,
                         (double)(i - 1) / (double)((NNHIST - 1)))) +
                    0.5);
    }
    for (int i = 2; i <= NNHIST; i++)
      if (mm[i] <= mm[i - 1] + 1)
    ...
    //////////////////////IMLI RELATED and backward/Forward
    /// history////////////////////////////////////
    current_hist.TaIMLI = 0; // use to monitor the iteration number (based on
                             // target locality for backward branches)
    current_hist.BrIMLI = 0; // use to monitor the iteration number (a second
                             // version based on backward branch locality))
    current_hist.F_TaIMLI =
        0; // use to monitor the iteration number,BHIST if TaIMLI = 0
    current_hist.F_BrIMLI = 0; // use to monitor the iteration number (a second
                               // version), FHIST if BrIMLI = 0
  ...
  int gindex(unsigned int PC, int bank, uint64_t hist,
             const tage_index_t &ch_i) {
    uint index;
    int logg = LOGG;
    int M = (m[bank] > PHISTWIDTH) ? PHISTWIDTH : m[bank];
    index = PC ^ (PC >> (abs(logg - bank) + 1)) ^ ch_i[bank].comp ^
            (ch_i[bank].comp >> logg) ^ F(hist, M, bank);
    uint32_t X =
  ...
  bool getbim() {
    BIM = (btable[BI].pred) ? (btable[BI >> HYSTSHIFT].hyst)
  ...
  // void Tagepred (uint64_t  PC)
  {
    ...
    for (int i = 1; i <= NHIST; i++) {
      ...
      GTAG[i] = gtag(PC, i, hist_to_use.phist, hist_to_use.ch_t[0],
                     hist_to_use.ch_t[1]);
    }
  ...
  uint64_t get_fourth_local_index(uint64_t PC) const {
    return (((PC ^ (PC >> (3)))) &
            (NQLOCAL - 1)); // different hash for the history
  }
  ...
  {
    pred_time_histories.insert(
        {get_unique_inst_id(seq_no, piece), active_hist});
  ...
  bool predict_using_given_hist(uint64_t seq_no, uint8_t piece,
    ...
    SUMSC = 0;
    ////// These extra counters seem to bring a marginal  gain of 0.006 MPKI
    /// when only pure SC, not useful when other info
#ifndef SCMEDIUM
    ...
    SUMGHIST += Gpredict(PC ^ (LongestMatchPred ^ (HCpred << 1)),
                         hist_to_use.GH, Am, AGEHL, ANB, LOGANB);
    predTSC = (SUMSC + SUMGHIST >= 0);
    // This is the contribution of "pure" global history

    SUMGHIST +=
        Gpredict(PC ^ (PC >> 3), hist_to_use.FHIST, Fm, FGEHL, FNB, LOGFNB);
    SUMGHIST +=
        Gpredict(PC ^ (PC >> 3), hist_to_use.PHIST, Pm, PGEHL, PNB, LOGPNB);
    if ((hist_to_use.TaIMLI < 12) && (hist_to_use.BrIMLI < 12) &&
        (hist_to_use.TaSIMLI < 12) && (hist_to_use.BrSIMLI < 12))
      SUMGHIST +=
          Gpredict(PC ^ (PC >> 6), hist_to_use.BHIST, Bm, BGEHL, BNB, LOGBNB);
#endif
#ifdef SCMEDIUM
    SUMFULL = 0;
#ifdef SCIMLI
...
#ifdef LOCALS
    LSUM += Gpredict(
        PC, hist_to_use.S_slhist[get_second_local_index(hist_to_use.PCBLOCK)],
        Sm, SGEHL, SNB, LOGSNB);
#endif
#ifdef LOCALT
    LSUM += Gpredict(PC, hist_to_use.T_slhist[get_third_local_index(PC)], Tm,
                     TGEHL, TNB, LOGTNB);
    LSUM += Gpredict(PC, hist_to_use.Q_slhist[get_fourth_local_index(PC)], Qm,
                     QGEHL, QNB, LOGQNB);
#endif
...
#ifdef EXTRAW
    LSUM = (((1 << PERCWIDTH) + WL[INDUPDS]) *
            ((1 << PERCWIDTH) + WL0[INDUPDS0]) * LSUM) >>
           (2 * PERCWIDTH - 1);
    SUMFULL = (((1 << PERCWIDTH) + WI[INDUPDS]) *
               ((1 << PERCWIDTH) + WI0[INDUPDS0]) * SUMFULL) >>
              (2 * PERCWIDTH - 1);
    ...
    int THRES = (Pupdatethreshold[INDUPDS] + Supdatethreshold[INDUPDS0] +
                 updatethreshold);
    pred_taken = (TAGECONF != 3) || (abs(SUMSC) >= THRES / 2)
                     ? SCPRED
                     : LongestMatchPred;
#else
    pred_taken = (SUMSC >= 0);
  ...
  void HistoryUpdate(uint64_t PCBRANCH, int brtype, bool taken,
    ...
    if (brtype & 1) {
      ...
      active_hist.S_slhist[get_second_local_index(active_hist.PCBLOCK)] =
          ((active_hist.S_slhist[get_second_local_index(active_hist.PCBLOCK)]
            << 1) +
           taken);
      active_hist.T_slhist[get_third_local_index(PC)] =
          ...
          (PC & 15);
      active_hist.Q_slhist[get_fourth_local_index(PC)] =
          (active_hist.Q_slhist[get_fourth_local_index(PC)] << 3) +
          (((active_hist.GH >> 2) & 3) << 1) + taken;
    ...
    if (taken)
      active_hist.PCBLOCK = branchTarget >> 2;
#ifdef SCMEDIUM
    if ((branchTarget >> 8) != active_hist.PrevRegion) {
      active_hist.PHIST = (active_hist.PHIST << 1) ^ (branchTarget >> 8);
      active_hist.PrevRegion = (branchTarget >> 8);
    }

    if (taken)
      if (branchTarget > PCBRANCH)
        active_hist.FHIST =
            (active_hist.FHIST << 3) ^ (branchTarget >> 2) ^ (PCBRANCH >> 1);
        // Caution: this  is quite  different from the Micro 2015 paper
        // rely on close targets  and on close  branches : capture loop nests,
        // see Tage Cookbook slide set
#define LOGREGSIZE 6
      ...
      if (taken) {
        if (branchTarget < PCBRANCH) {
          ...
          if (((branchTarget & 65535)) == active_hist.SLastBack) {
            ...
            active_hist.BHIST =
                (active_hist.BHIST << 1) ^ active_hist.SLastBack;
            active_hist.TaSIMLI = 0;
          }
...

          {
            ...
            active_hist.BHIST =
                (active_hist.BHIST << 1) ^ active_hist.SLastBackPC;
    ...
    active_hist.F_TaIMLI =
        (active_hist.TaIMLI == 0) || (active_hist.BrIMLI == active_hist.TaIMLI)
            ? (active_hist.BHIST & 63)
            : active_hist.TaIMLI;
    active_hist.F_BrIMLI = (active_hist.BrIMLI == 0) ? (active_hist.FHIST & 63)
                                                     : active_hist.BrIMLI;

    active_hist.F_TaSIMLI = (active_hist.TaSIMLI == 0) ||
                                    (active_hist.BrSIMLI == active_hist.TaSIMLI)
                                ? (active_hist.BHIST & 7)
                                : active_hist.TaSIMLI;
    active_hist.F_BrSIMLI = (active_hist.BrSIMLI == 0) ? (active_hist.FHIST & 7)
                                                       : active_hist.BrSIMLI;
  }
  ...
  void update(uint64_t PCBRANCH, bool resolveDir, bool pred_taken,
...

    {
      if ((LSUM >= 0) != (SUMSC - LSUM >= 0))
        ctrupdate(WL[INDUPDS], ((LSUM >= 0) == resolveDir), PERCWIDTH);
      if ((LSUM >= 0) != (SUMSC - LSUM >= 0))
        ctrupdate(WL0[INDUPDS0], ((LSUM >= 0) == resolveDir), PERCWIDTH);

      if ((SUMFULL >= 0) != (SUMSC - SUMFULL >= 0))
        ctrupdate(WI[INDUPDS], ((SUMFULL >= 0) == resolveDir), PERCWIDTH);
      if ((SUMFULL >= 0) != (SUMSC - SUMFULL >= 0))
        ctrupdate(WI0[INDUPDS0], ((SUMFULL >= 0) == resolveDir), PERCWIDTH);
...
#define OPTTHRESUPD
      ...
      Gupdate(PC ^ (LongestMatchPred ^ (HCpred << 1)), resolveDir,
              hist_to_use.GH, Am, AGEHL, ANB, LOGANB);

      Gupdate(PC ^ (PC >> 3), resolveDir, hist_to_use.FHIST, Fm, FGEHL, FNB,
              LOGFNB);
      //    if ((hist_to_use.TaIMLI <12) && (hist_to_use.BrIMLI <12) &&
      //    (hist_to_use.TaSIMLI <12) && (hist_to_use.BrSIMLI <12))
      Gupdate(PC ^ (PC >> 3), resolveDir, hist_to_use.PHIST, Pm, PGEHL, PNB,
              LOGPNB);
      if ((hist_to_use.TaIMLI < 12) && (hist_to_use.BrIMLI < 12) &&
          (hist_to_use.TaSIMLI < 12) && (hist_to_use.BrSIMLI < 12))
        Gupdate(PC ^ (PC >> 6), resolveDir, hist_to_use.BHIST, Bm, BGEHL, BNB,
                LOGBNB);
...
#endif
#ifdef LOCALS
      Gupdate(PC, resolveDir,
              hist_to_use.S_slhist[get_second_local_index(hist_to_use.PCBLOCK)],
              Sm, SGEHL, SNB, LOGSNB);
#endif
#ifdef LOCALT
      ...
      Gupdate(PC, resolveDir, hist_to_use.T_slhist[get_third_local_index(PC)],
              Tm, TGEHL, TNB, LOGTNB);
      Gupdate(PC, resolveDir, hist_to_use.Q_slhist[get_fourth_local_index(PC)],
              Qm, QGEHL, QNB, LOGQNB);
#endif
      ...
      if (PseudoNewAlloc) {
        ...
        for (int i = HitBank / 4; i <= NHIST / 4; i++) {
          ...
          if ((LongestMatchPred != resolveDir) || ((MYRANDOM() & 31) > 1)) {
            ctrupdate(COUNT16_31[INDCOUNT][i], (resolveDir == LongestMatchPred),
                      7);
          ...
          if ((LongestMatchPred != resolveDir) || ((MYRANDOM() & 31) > 3)) {
            ctrupdate(COUNT8_15[INDCOUNT][i], (resolveDir == LongestMatchPred),
                      7);
    ...
    //  when allocating a new entry is unlikely to result in a good prediction,
    //  rarely allocate
    if (TAGECONF < 2) {
      if ((COUNT50[INDCOUNT][(HitBank + 1) / 4] < 0) &
          (Count50[(HitBank + 1) / 4] < 0)) {
        ALLOC &= ((MYRANDOM() & ((1 << (4 - TAGECONF)) - 1)) == 0);
      } else
        // the future allocated entry is not that likely to be correct
        if ((COUNT16_31[INDCOUNT][(HitBank + 1) / 4] < 0) &
            (Count16_31[(HitBank + 1) / 4] < 0)) {
          ALLOC &= ((MYRANDOM() & ((1 << (2 - TAGECONF)) - 1)) == 0);
        } else if ((COUNT8_15[INDCOUNT][(HitBank + 1) / 4] < 0) &
    ...
    if (ALLOC) {
      ...
      int MaxNALLOC = (TAGECONF) +
                      (!((COUNT50[INDCOUNT][(HitBank + 1) / 4] < 0) &
                         (Count50[(HitBank + 1) / 4] < 0))) +
                      (!((COUNT16_31[INDCOUNT][(HitBank + 1) / 4] < 0) &
                         (Count16_31[(HitBank + 1) / 4] < 0))) +
                      (!((COUNT8_15[INDCOUNT][(HitBank + 1) / 4] < 0) &
                         (Count8_15[(HitBank + 1) / 4] < 0)));
      ...
      // we set two counts to monitor: "time to reset u" and "almost time reset
      // u": TICKH is useful only if UWIDTH =1
      ...
      if (TICK < 0)
        TICK = 0;
      if (TICK >= BORNTICK) {
        for (int j = 0; j < ASSOC * (1 << LOGG) * NBANK; j++) {
...
#define GINDEX                                                                 \
  (((long long)PC) ^ bhist ^ (bhist >> (8 - i)) ^ (bhist >> (16 - 2 * i)) ^    \
   ...
   (bhist >> (40 - 4 * i)) ^ (bhist >> (48 - 4 * i))) &                        \
      ((1 << (logs /*- (i >= (NBR - 2))*/)) - 1)
  int Gpredict(uint64_t PC, uint64_t BHIST, int *length, int8_t **tab, int NBR,
               int logs) {
    int PERCSUM = 0;
    for (int i = 0; i < NBR; i++) {
